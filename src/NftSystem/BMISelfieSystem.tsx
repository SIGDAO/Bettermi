...
function isToday(date: string): boolean {
  ...
  return givenDate.getDate() === today.getDate() && givenDate.getMonth() === today.getMonth() && givenDate.getFullYear() === today.getFullYear();
}
...
export async function CheckTakenSelfie(userAccountId: string, ledger2: any, BMIContractCodeHashId: string, nftDistributor: string) {
  ...
  for (var i = unconfirmedTransactionList.length - 1; i >= 0; i--) {
    if (unconfirmedTransactionList[i].recipient === BMIContract.ats[0].at && unconfirmedTransactionList[i].sender === userAccountId) {
  ...
  for (let i = 0; i < messagesOnChain.transactions.length; i--) {
    ...
    if (messagesOnChain.transactions[i].recipient === BMIContract.ats[0].at && messagesOnChain.transactions[i].sender === userAccountId) {
      //Also need to check sender
...
export async function CheckIsUserFirstDayOfRegistration(ledger2: any, userId: String, codeHashIdForContract: string) {
  ...
  if (senderNftStorage.ats.length === 0) {
    ...
    const contractInfo = await ledger2.contract.getContract(senderNftStorage.ats[0].at);
